...
const App = () => {
  const {rName, setRnamed} = useState('RNAME');
    ...
    mediaDevices.enumerateDevices().then(mediaInfos => {
      ...
      for (let i = 0; i < mediaInfos.length; i++) {
        ...
        if (
          ...
          mediaInfo.facing == (isFront ? 'front' : 'environment')
        ) {
  ...
  return indexIncom === false ? (
    <ScrollView>
      {console.log(
        `partner: ${partnerVideo?.toURL()}`,
        `user: ${userVideo?.toURL()}`,
      )}
      <View>
        {partnerVideo !== null ? (
          <RTCView
